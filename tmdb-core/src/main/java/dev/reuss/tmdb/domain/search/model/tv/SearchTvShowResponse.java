package dev.reuss.tmdb.domain.search.model.tv;

import com.fasterxml.jackson.annotation.JsonIgnoreProperties;
import com.fasterxml.jackson.annotation.JsonProperty;
import dev.reuss.tmdb.common.TmdbModel;
import dev.reuss.tmdb.common.page.PagedResponse;
import dev.reuss.tmdb.util.TmdbCollections;

import java.util.List;

/**
 * Paginated TMDB TV show search response.
 *
 * @param page         current result page
 * @param results      found TV shows
 * @param totalPages   total available pages
 * @param totalResults total available results
 */
@JsonIgnoreProperties(ignoreUnknown = true)
public record SearchTvShowResponse(
        int page,

        List<SearchTvShow> results,

        @JsonProperty("total_pages")
        int totalPages,

        @JsonProperty("total_results")
        int totalResults
) implements PagedResponse<SearchTvShow>, TmdbModel {

    public SearchTvShowResponse {
        results = TmdbCollections.list(results);
        ;
    }
}